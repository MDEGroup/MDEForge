...
//		filetypeField.setBoost(1.4f);
	...
	// EcoreMetamodel emm =
	// ((EcoreMetamodel)model.getMetamodel().getToArtifact());
	// ecoreMetamodelService.loadArtifact(emm);
	// ResourceSet load_resourceSet = new ResourceSetImpl();
	//
	...
	// load_resourceSet.getResource(URI.createURI(gridFileMediaService.getFilePath(model)),
	// true);
	//
	// TreeIterator<EObject> eAllContents = load_resource.getAllContents();
	// while (eAllContents.hasNext()) {
	...
	// * Index className:classAttributeValue
	// */
	// Field eClassWithAttributeField = new Field(eClass.getName(),
	// attributeValue, Store.YES, Index.ANALYZED);
	// // eClassWithAttributeField(1.5f);
	// doc.add(eClassWithAttributeField);
	//
	// /*
	// * Index className::classAttribute:attributeValue
	// */
	// Field eClassWithAttributeAndAttributeValueField = new
	// Field(eClass.getName() + CUSTOM_LUCENE_INDEX_SEPARATOR_CHARACTER +
	// attribute.getName(), attributeValue, Store.YES, Index.ANALYZED);
	// // eClassWithAttributeAndAttributeValueField(1.5f);
	// doc.add(eClassWithAttributeAndAttributeValueField);
	// }
	//
	...
	// String key = reference.getName();
	// EClass referenceTo = (EClass) value.eClass();
	// Field eClassReferenceField = new Field(key, referenceTo.getName(),
	...
	//
	// }
	// }catch(Exception e) { logger.error("Some error when try to parse EMF
	// index");}
	// //Artifact TYPE: "Model"
	// Field artifactType = new Field(TYPE_TAG,
	// model.getClass().getSimpleName(), Store.YES, Index.ANALYZED);
	// doc.add(artifactType);
	//
	//// String text = handler.toString();
	// String text =
	// getTextFromInputStream(gridFileMediaService.getFileInputStream(model));
	...
	//
	// String artifactName = model.getName();
	// Field artName = new Field(NAME_TAG, artifactName, Store.YES,
	// Index.ANALYZED);
	//// filenameField.setBoost(0.5f);
	//
	// String author = model.getAuthor().getUsername();
	// Field authorField = new Field(AUTHOR_TAG, author, Store.YES,
	// Index.ANALYZED);
	//
	// Date lastUpdate = model.getModified();
	...
	// doc.add(propField);
	// }
	// Field idField = new Field(ID_TAG, model.getId(), Store.YES,
	// Index.ANALYZED);
	//
	//
	// Field conformToFieldName = new Field(CONFORM_TO_TAG,
	// model.getMetamodel().getToArtifact().getName(), Store.YES,
	// Index.ANALYZED);
	// doc.add(conformToFieldName);
	// Field conformToFieldId = new Field(CONFORM_TO_TAG,
	// model.getMetamodel().getToArtifact().getId(), Store.YES, Index.ANALYZED);
	// doc.add(conformToFieldId);
	// doc.add(textField);
	// doc.add(artName);
	// doc.add(authorField);
