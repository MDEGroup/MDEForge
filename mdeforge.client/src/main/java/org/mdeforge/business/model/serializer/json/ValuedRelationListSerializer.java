...
public class ValuedRelationListSerializer extends JsonSerializer<List<ValuedRelation>> {

	@Override
	public void serialize(List<ValuedRelation> rel, JsonGenerator jgen,
			SerializerProvider arg2) throws IOException,
			JsonProcessingException {
		jgen.writeStartArray();
		if (rel != null && rel.size() != 0) {
			for (ValuedRelation relation : rel) {
				if (relation != null) {
					jgen.writeStartObject();
					jgen.writeStringField("id", relation.getId());
					jgen.writeNumberField("value", relation.getValue());
